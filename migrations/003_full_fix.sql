-- ============================================================
-- SportXtreme Events — Full Schema + RLS Fix
...
-- ── gallery table (MISSING — create now) ─────────────────────
...
DROP TRIGGER IF EXISTS update_gallery_modtime ON public.gallery;
CREATE TRIGGER update_gallery_modtime
  BEFORE UPDATE ON public.gallery
  FOR EACH ROW EXECUTE FUNCTION update_modified_column();

-- ── testimonials table (MISSING — create now) ────────────────
...
-- ── DROP all existing policies (clean slate, no duplicates) ──
DO $$ DECLARE r record;
BEGIN
  FOR r IN
...
-- ── PUBLIC INSERT: contacts & proposals (fixes 42501) ────────
...
-- ── PUBLIC SELECT: events, gallery, testimonials ─────────────
...
-- ── ADMIN FULL ACCESS (sportxtremeevents@gmail.com) ──────────
